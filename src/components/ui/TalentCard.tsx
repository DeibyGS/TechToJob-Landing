...
export function TalentCard({
  ...
  const rows = [
    ...
    { icon: Clock, label: availabilityLabel, value: availabilityValue },
  ];

  return (
    <div className="relative w-full max-w-md overflow-hidden rounded-xl border border-brand-dark/10 bg-white p-6 shadow-[0_2px_8px_rgba(47,52,54,0.06),0_16px_40px_rgba(47,52,54,0.06)] lg:max-w-none">
      {/* Watermark logo */}
      <svg
      ...
      <div className="flex flex-col gap-5">
        {rows.map(({ icon: Icon, label, value }) => (
          <div key={label} className="flex items-center gap-3">
            <div className="flex h-10 w-10 shrink-0 items-center justify-center rounded-full bg-brand-dark/5">
              <Icon className="h-5 w-5 text-brand-dark/70" aria-hidden="true" />
            </div>
            <div>
