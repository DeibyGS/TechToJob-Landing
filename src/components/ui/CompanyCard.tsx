import { Code2, TrendingUp, Briefcase } from "lucide-react";
...
export function CompanyCard({
  ...
  const rows = [
    { icon: Code2, label: stackLabel, value: stackValue },
    { icon: TrendingUp, label: levelLabel, value: levelValue },
    { icon: Briefcase, label: engagementLabel, value: engagementValue },
  ];

  return (
    <div className="relative w-full max-w-md overflow-hidden rounded-xl border border-brand-dark/10 bg-brand-teal p-6 shadow-[0_2px_8px_rgba(47,52,54,0.10),0_16px_40px_rgba(47,52,54,0.10)] lg:max-w-none">
      {/* Watermark logo — dark, not teal-on-teal, so it stays visible against
          the card's own teal fill. */}
      <svg
        className="absolute -right-6 top-0 h-full w-auto text-brand-dark/10"
      ...
      <div className="flex flex-col gap-5">
        {rows.map(({ icon: Icon, label, value }) => (
          <div key={label} className="flex items-center gap-3">
            <div className="flex h-10 w-10 shrink-0 items-center justify-center rounded-full bg-brand-dark/10">
